...
class Category(ABC):
    ...
    def _custom_check(self, transaction) -> bool:
       if self.additional_checks:
          return any([func(transaction) for func in self.additional_checks])
       
    def check(self, transaction) -> bool:
      return  self._keyword_match(transaction) or self._custom_check(transaction)