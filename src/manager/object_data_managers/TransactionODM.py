...
from lib.data_models import Transaction, TimeStamp, Account

class TransactionODM(Transaction, Document):
    class Settings:
        name = 'Transactions'
        bson_encoders = {
          date: str
...
if __name__ == '__main__':
    ...
    init_bunnet(database=client.therealdeal, document_models=[TransactionODM]) # add documents here


    chase = Account(name="Chase", parent_item= "chase_bank")
    print(TimeStamp())
    t = TransactionODM(account=chase, amount=9, category= 'N/A', date_authorized=TimeStamp(dt="2024-10-17"), name='volo1')
