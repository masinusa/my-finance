...
def update_transactions():
  ...
  for item in a_tokens:
    ...
    # Request transactions for current instiution
    try:
    ...
    # Update cursor
    req_json = {'institution': item['institution'], 'transaction_cursor': str(new_cursor)}
    _ = requests.put("http://db_connector:5000/database/transaction_cursors", json=req_json)
    assert(_.ok)
